...
const ViewResult = () => {
  ...
  useEffect(() => {
    if (location.state?.result) {
      ...
      // If no result passed via navigation, try to get the latest from history
      const getLatestResult = async () => {
        const existingResults = JSON.parse(localStorage.getItem('testResults') || '[]');
        if (existingResults.length > 0) {
          setResult(existingResults[existingResults.length - 1]);
        }
      };
      getLatestResult();
    }
      ...
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2 }}>
        ...
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, mb: 2 }}>
            <Chip label={result.topic} color="primary" />
            <Chip label={result.difficulty} color="secondary" />
            <Chip label={`${result.score}/${result.total} Correct`} 
